...
class Requester {
	...
	execute() {
		...
		.then(response => {
			...
			if (process.env.FRISKY_DEBUG) console.log("Received response with "+Object.values(data).reduce((acc, cur) => acc + cur.length, 0)+" items from "+Object.keys(data).length+" models")
